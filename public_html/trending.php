...
if($result && mysqli_num_rows($result) > 0) {
    while($row = mysqli_fetch_assoc($result)) {
        echo '<div class="rounded-0 mb-2">';
        echo '<div class="d-flex align-items-center mb-2">';
        echo '<a href="allprofile.php?user='.$row['thread_user_id'].'" class="text-dark" style="text-decoration:none;">';
        if(!empty($row['profile_pic'])){
            echo '<img src="img/images/'. htmlspecialchars($row['profile_pic']) .'" alt="'. htmlspecialchars($row['about']) .'" width="50px" height="50px" class="rounded-circle">';
        } else { 
        ...
        echo '<div class="ms-2">';
        echo '<a href="allprofile.php?user='. $row['thread_user_id'] .'" class="text-dark" style="text-decoration:none;">';
        echo '<span class="fw-bold" style="font-size: 1.1rem;">'. htmlspecialchars($row['username']) .'</span>';
        ...
        echo '<h5>'. htmlspecialchars($row['thread_name']) .'</h5>';
        echo '</a>';
        ...
        echo '</div>';
        echo '<div>';
        echo '<span><a href="thread.php?thread='.$row['thread_id'].'" class="text-dark" style="text-decoration:none;"><i class="ri-chat-3-line"></i> '.$row['comment_count'].' &#183 Comments </a></span>';
        echo '</div>';
        echo '</div>';
