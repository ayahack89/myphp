                         ...
                         if ($method == 'POST') {
                              ...
                              if (isset($_POST['submit'])) {
                                   ...
                                   if (!empty($image_name)) {
                                        ...
                                        // Check file size
                                        if ($image_size > 999998) { // 500KB limit
                                             ...
                                             if (!in_array($image_extension, $allowed_types)) {
                                                  ...
                                                  if (move_uploaded_file($tmp_name, $image_path_destination . $image_new_name)) {
                                                       ...
                                                       if (mysqli_num_rows($user) > 0) {
                                                            ...
                                                            $sql_query = "INSERT INTO `threads` (`thread_name`, `thread_desc`, `thread_catagory_id`, `thread_user_id`, `uploaded_image`) VALUES ('{$topic_name}', '{$topic_desc}', '{$get_id}', '{$thread_user_id}', '{$image_new_name}' )";
                                                            $result = mysqli_query($conn, $sql_query);
                                                            if ($result) {
                                                                 $alert = true;
                                                            } else {
     ...
     if ($result) {
          ...
          if (mysqli_num_rows($result) > 0) {
               ...
               while ($thread = mysqli_fetch_assoc($result)) {
                    ...
                    $thread_name = $thread['thread_name']; // Corrected variable name
                              ...
                              <div class="container d-flex bg-light border py-4 px-4">
                                   ...
                                   <div class="text">
                                        <h5>
                                             ...
                                             <b style="font-size:11px; font-weight:lighter;">created :
